...
public class ShootOnTheFlyCalculator {
  ...
  public static Pose3d calculateEffectiveTargetLocation(
    ...
    for (int i = 0; i < goalPositionIterations; i++) {
      double virtualGoalX = targetPose.getX() - shotTime * (fieldRelRobotVelocity.vxMetersPerSecond
          + fieldRelRobotAcceleration.axMetersPerSecondSquared * accelerationCompensationFactor);
      double virtualGoalY = targetPose.getY() - shotTime * (fieldRelRobotVelocity.vyMetersPerSecond
          + fieldRelRobotAcceleration.ayMetersPerSecondSquared * accelerationCompensationFactor);

      correctedTargetPose = new Pose3d(virtualGoalX, virtualGoalY, targetPose.getZ(), targetPose.getRotation());
  ...
  public static InterceptSolution solveShootOnTheFly(
    ...
    ShotSolution sol = BallPhysics.solveBallisticWithSpeed(robotPose, targetPose, targetSpeedRps);
    ...
    for (int i = 0; i < maxIterations; i++) {
      ...
      ShotSolution newSol = BallPhysics.solveBallisticWithSpeed(robotPose, effectiveTarget, targetSpeedRps);
    ...
    return new InterceptSolution(effectiveTarget, sol.launchPitchRad(), sol.launchSpeed(), sol.flightTimeSeconds(), 0);
  }
}