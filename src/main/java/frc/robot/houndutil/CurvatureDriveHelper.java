...
public class CurvatureDriveHelper {
  ...
  public DifferentialDriveWheelSpeeds cheesyDrive(double throttle, double wheel, boolean isQuickTurn, double maxSpeed) {
    ...
    if (isQuickTurn) {
      if (Math.abs(throttle) < 0.2) {
        double alpha = 0.1;
        mQuickStopAccumulator = (1 - alpha) * mQuickStopAccumulator + alpha * MathUtil.clamp(wheel, -1.0, 1.0) * 2;
      }
      overPower = 1.0;
      angularPower = wheel;
    } else {
