...
public class AprilTagPhotonCamera {
   ...
   */
  public AprilTagPhotonCamera(
      String name,
      Transform3d robotToCam,
      PhotonCameraConstants constants,
      double avgErrorPx,
      double stdDevErrorPx,
    ...
    trigSolvePoseEstimator = new PhotonPoseEstimator(fieldLayout, PoseStrategy.PNP_DISTANCE_TRIG_SOLVE, robotToCam);

    if (RobotBase.isSimulation()) {
      var cameraProp = new SimCameraProperties();
      cameraProp.setCalibration(constants.WIDTH, constants.HEIGHT, Rotation2d.fromDegrees(constants.FOV));
  ...
  private void processEstimatedGlobalPose(
    ...
    if (photonEstimatedRobotPose.isPresent()) {
      ...
      detectedAprilTags = getPosesFromTargets(result.targets, estimatedRobotPose, robotToCam);
      ...
      Matrix<N3, N1> stddevs = getEstimationStdDevs(result, estimatedRobotPose, singleTagStdDevs, multiTagStdDevs);

      estimatedRobotPoses.add(new Pair<EstimatedRobotPose, Matrix<N3, N1>>(photonEstimatedRobotPose.get(), stddevs));
    } else {
  ...
  private void processEstimatedTrigPose(PhotonPipelineResult result) {
    ...
    if (photonEstimatedRobotPose.isPresent()) {
      ...
      detectedTrigAprilTags = getPosesFromTargets(result.targets, estimatedTrigPose, robotToCam);
   ...
   */
  protected Pose3d[] getPosesFromTargets(
