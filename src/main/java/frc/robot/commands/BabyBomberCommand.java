 ...
 */
public class BabyBomberCommand extends Command {
  ...
  private boolean hasBabyBirded = false;
  private boolean sensorTripped = false;
  private boolean sensorCleared = false;

  public BabyBomberCommand(CommandSwerveDrivetrain drivetrain, ShooterSubsystem shooter,
  ...
  public void execute() {
    if (!hasBabyBirded) {
      ...
      if (sensorTripped && !turretSubsystem.hasNote()) {
        turretSubsystem.runRollers(INTAKE_VELOCITY);
        shooter.stop();
        sensorCleared = true;
      }

      if (sensorTripped && sensorCleared && turretSubsystem.hasNote()) {
        hasBabyBirded = true;
      }
  ...
  public boolean isFinished() {
    return (!turretSubsystem.hasNote() && hasBabyBirded && isShooting);
  }
